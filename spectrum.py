...
if selected_dimension == "Auction Integrated": #This is the new dimension that is being tested
	...
	for i, (band, df) in enumerate(non_empty_dfs.items(), start=1):
		...
		fig.update_xaxes(row=i, col=1, tickfont=dict(size=text_embed_in_chart_size*0.5))
	...
	fig.update_layout(uniformtext_minsize=text_embed_in_chart_size, 
		...
		width=heatmapwidth,
		height=heatmapheight * len(non_empty_dfs)*0.15,  # Total height based on the number of subplots
		autosize=True,
