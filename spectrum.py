...
def summarychart(summarydf, xcolumn, ycolumn):
	...
	text = bar.mark_text(size = 15, dx=0, dy=-7, color = 'white').encode(text=ycolumn+':Q')
...
if Dimension == "Calendar Year":
	...
	if Feature == "Auction/Reserve":
		...
		df1_temp2["Auction/Reserve"] = np.divide(df1[feature_dict["Auction Price"],df1[feature_dict["Reserve Price"]],out=np.full_like(df1[feature_dict["Auction Price"]], np.nan), where=df1[feature_dict["Reserve Price"]] != 0)
