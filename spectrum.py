...
if selected_dimension == "Auction Integrated": #This is the new dimension that is being tested
	...
	fig = make_subplots(rows=len(df_dict), cols=2, specs=[[{"type": "heatmap"}, {"type": "bar"}] for _ in range(len(df_dict))],
						...
						horizontal_spacing=0.02,  # Set minimal horizontal spacing between columns
						column_widths=[0.9, 0.1])  # Adjust the width of columns if necessary
	...
	for i, (band, df_segment) in enumerate(df_dict.items(), start=1):
		...
		fig.update_yaxes(row=i, col=1, fixedrange=True, showline=True, linewidth=1.5, linecolor='black', mirror=True, showgrid=True, gridcolor='lightgrey')
